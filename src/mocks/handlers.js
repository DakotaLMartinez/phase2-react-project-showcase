import { rest } from "msw";

let lastId = 25;
const projects = [
  {
    id: 1,
    name: "Great Outdoors Guide",
    about: "Plan your next adventure to in the U.S. National Parks!",
    phase: 4,
    link: "https://great-outdoors-guide.netlify.app",
    image: "https://i.imgur.com/8GnP2Uw.png",
    claps: 0,
  },
  {
    id: 2,
    name: "Book Her #2",
    about:
      "The app takes you (the Casting Director) through a series of questions to determine the Models you need for your next Project.",
    phase: 4,
    link: "https://bookher.netlify.app/new_project",
    image: "https://i.imgur.com/INs84Uk.png",
    claps: 5,
  },
  {
    id: 3,
    name: "Code Bucket",
    about: "Web-based CSS, JS and HTML Editor",
    phase: "2",
    link: "https://code-bucket.netlify.app",
    image: "https://i.imgur.com/Hz84TA2.png",
    claps: 7,
  },
  {
    id: 11,
    name: "Yourseum",
    about: "Find great art",
    phase: 4,
    link: "https://yourseum.netlify.app",
    image: "https://i.imgur.com/yywQCoi.png",
    claps: 0,
  },
  {
    id: 12,
    name: "Spreddit",
    about: "A Global Community of Like Minded Humans Socially Distancing ",
    phase: 4,
    link: "https://spreddit.netlify.app",
    image: "https://i.imgur.com/cyZCpWs.png",
    claps: 0,
  },
  {
    id: 13,
    name: "snackATTACK",
    about: "It's Time to Discover",
    phase: 4,
    link: "https://snackattack.netlify.app",
    image: "https://i.imgur.com/VTJB4qy.png",
    claps: 0,
  },
  {
    id: 20,
    name: "YouTravel",
    about: "YouTube for Travel Videos",
    phase: 3,
    link: "https://youtravel.netlify.app/",
    image: "https://i.imgur.com/Y6mlORn.png",
    claps: 0,
  },
  {
    id: 22,
    name: "Breathe",
    about: "Meditation",
    phase: 3,
    link: "https://github.com/maifam/breathe_frontend",
    image: "https://i.imgur.com/hAuKx3K.png",
    claps: 0,
  },
  {
    id: 23,
    name: "HomeStory",
    about:
      "A web app for storing info about furniture items you plan on purchasing in the future or simply just want to keep tabs of.",
    phase: 3,
    link: "https://homestory.netlify.app",
    image: "https://i.imgur.com/il8ft21.png",
    claps: 0,
  },
  {
    id: 24,
    name: "Locksley To Do",
    about: "A todo list and calendar app",
    phase: 4,
    link: "https://www.locksleyr.com",
    image: "https://i.imgur.com/XOnaclL.png",
    claps: 0,
  },
  {
    id: 25,
    name: "GamesRx",
    about: "Game info and reviews",
    phase: 4,
    link: "https://gamesrx.netlify.app",
    image: "https://i.imgur.com/Japzh6v.png",
    claps: 0,
  },
];
export const handlers = [
  rest.get("http://localhost:4000/projects", (req, res, ctx) => {
    const query = req.url.searchParams.get("q") || "";
    const phase = req.url.searchParams.get("phase");
    const limit = req.url.searchParams.get("_limit");
    const order = req.url.searchParams.get("_order") || "asc";
    const sort = req.url.searchParams.get("_sort");
    let results = projects.filter((project) => {
      return (
        project.name.toLowerCase().includes(query.toLowerCase()) ||
        project.about.toLowerCase().includes(query.toLowerCase())
      );
    });
    if (phase) {
      results = results.filter((project) => {
        return project.phase === parseInt(phase);
      });
    }
    if (sort) {
      results.sort((a, b) => {
        if (order === "asc") {
          return a[sort] - b[sort];
        } else {
          return b[sort] - a[sort];
        }
      })
    }
    if (limit) {
      results = results.slice(0, parseInt(limit))
    }
    return res(ctx.status(200), ctx.json(results));
  }),
  rest.get("http://localhost:4000/projects/:id", (req, res, ctx) => {
    return res(
      ctx.status(200),
      ctx.json(projects.find((p) => p.id === parseInt(req.params.id)))
    );
  }),
  rest.post("http://localhost:4000/projects", async (req, res, ctx) => {
    const project = await req.json();
    return res(ctx.status(201), ctx.json({ id: ++lastId, ...project }));
  }),
  rest.patch("http://localhost:4000/projects/:id", async (req, res, ctx) => {
    const updatedProject = await req.json();
    const projectIndexToUpdate = projects.findIndex(
      (p) => p.id === parseInt(req.params.id)
    );
    projects[projectIndexToUpdate] = updatedProject;
    return res(ctx.status(200), ctx.json(updatedProject));
  }),
  rest.delete("http://localhost:4000/projects/:id", (req, res, ctx) => {
    const projectIndexToDelete = projects.findIndex(
      (p) => p.id === parseInt(req.params.id)
    );
    projects.splice(projectIndexToDelete, 1);
    return res(ctx.status(204));
  }),
];
